...
$(document).ready(function () {
  ...
  $(".folder_toggle").click(function () {
    ...
    $.ajax({
      url: "files/control/newcontent.php?folder=" + $folder,
      success: function () {
        $("#toggle_show" + $i)
          .empty()
          .load("files/control/newcontent.php?folder=" + $folder);
      },
    });
  });
  ...
  function capture() {
    ...
    html2canvas($("#preview_front"), {
      onrendered: function (canvas) {
        ...
        $("#img_front").val(canvas.toDataURL("image/jpg"));
      },
    });
    //$('#preview_front').hide();
    //$('#preview_back').show();
    html2canvas($("#preview_back"), {
      onrendered: function (canvas) {
        ...
        document.getElementById("image_reply").appendChild(canvas);
        $("#img_back").val(canvas.toDataURL("image/jpg"));
        $("#preview_back").addClass("dis_none");
      },
    });
  }
  $("#download_button").click(function () {
    // Get the canvas elements containing the images
    var canvasFront = document.querySelector(
      "#image_reply canvas:nth-child(1)"
    );
    var canvasBack = document.querySelector("#image_reply canvas:nth-child(2)");
    ...
    anchor.href = combinedCanvas.toDataURL("image/jpg");
...
function image_icon($srcimg) {
  $("." + $y_pos + "_print").append(
    "<div id=icon" +
      $nos_icons +
      " class='new_icon' onmouseover='show_delete_btn(this);' onmouseout='hide_delete_btn(this);'><span class='delete_icon property_icon' onClick='delete_icons(this);'></span><img src='" +
      $srcimg +
